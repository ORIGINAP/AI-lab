...
num_ftrs = model_conv.classifier[1].in_features # EfficientNet uses classifier[1] as the final linear layer
model_conv.classifier[1] = nn.Linear(num_ftrs, 2) # Correctly replace the classification layer
...
# opposed to before.
optimizer_conv = optim.SGD(model_conv.classifier[1].parameters(), lr=0.001, momentum=0.9) # Optimize the newly added layer
